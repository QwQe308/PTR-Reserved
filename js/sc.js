const SOFTCAPS = {
	/*c2: {
		title: "文明效果 2 ",
		type: "root",
		start() { return new Decimal("10000") },
		exp: new Decimal(1),
		display() { return tmp.c.eff2.gte(this.start()) },
		info() { return "开始于 "+format(this.start())+"x, 立方根" },
	},*/
	c2: {
		title: "文明效果 3 ",
		type: "root",
		start() { return new Decimal("1000") },
		mag: new Decimal(3),
		display() { return tmp.c.eff2.gte(this.start()) },
		info() { return "开始于 "+format(this.start())+"x, 立方根" },
	},
	AI_BA: {
		...
		start() { return new Decimal("1000") },
		mag: new Decimal(3),
		display() { return upgradeEffect("ai",21).gte(this.start()) },
		info() { return "开始于 "+format(this.start())+"x, 指数立方根" },
	},
	AI_C: {
		title: "AI文明",
		type: "root",
		start() { return new Decimal("5") },
		mag: new Decimal(3),
		display() { return buyableEffect("ai",13).gte(this.start()) },
		info() { return "开始于 "+format(this.start())+"x, 立方根" },
	},
	points1: {
		title: "点数获取",
		type: "expRoot",
		start: new Decimal("e2000"),
		mag: new Decimal(2),
		display() { return player.points.gte("e2000") },
