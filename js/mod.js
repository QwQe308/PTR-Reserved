var one = new Decimal(1) // Omega树改为 var one = new OmegaNum(1)
...
function logsoftcap(num,start,power){
    if(num.gt(start)){
        num = ten.tetrate(num.slog(10).sub(power)).pow(start.log(ten.tetrate(start.slog(10).sub(power))))
    }
    return num
}
...
function getPointGen() {
	...
	let gain = tmp.c.eff1
	gain = gain.mul(tmp.c.eff2)
	gain = gain.mul(tmp.c.eff3)
	gain = gain.mul(tmp.ai.conscEff1)
